            ...
            <DialogContent className="max-w-2xl">
              ...
              {selectedUserForBranches && (
                <div className="space-y-4">
                  ...
                  {(() => {
                    // ⭐ กรองสาขา: Developer = ทุกสาขา, คนอื่น = เฉพาะที่เป็นเจ้าของหรือมีสิทธิ์
                    const myAccessibleBranches = currentUser?.accessible_branches;
                    const isDeveloper = userRole === 'developer';
                    const hasAccessibleBranchesSet = myAccessibleBranches !== null && myAccessibleBranches !== undefined;
                    
                    const filteredBranches = (() => {
                      if (isDeveloper) return branches;
                      
                      // ถ้ามี accessible_branches set = ใช้ลิสต์นั้น
                      if (hasAccessibleBranchesSet) {
                        return branches.filter(b => myAccessibleBranches.includes(b.id));
                      }
                      
                      // ถ้าไม่มี set = แสดงเฉพาะสาขาที่เป็นเจ้าของ
