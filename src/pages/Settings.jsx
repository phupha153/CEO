                    ...
                    <Tabs defaultValue="line" className="w-full">
                      ...
                      <TabsContent value="line" className="space-y-6">
                        ...
                        {(userRole === 'developer' || userRole === 'owner') && (
                          <div className="bg-gradient-to-br from-indigo-50 to-blue-50 rounded-xl p-5 border border-indigo-100 shadow-sm relative overflow-hidden"><div className="absolute -right-4 -top-4 w-24 h-24 bg-indigo-100 rounded-full opacity-50 blur-xl"/><div className="flex items-start gap-4 relative z-10"><div className="bg-indigo-100 p-3 rounded-full flex-shrink-0 mt-1 shadow-sm border border-indigo-200"><MessageSquare className="w-6 h-6 text-indigo-600"/></div><div className="flex-1"><h4 className="text-lg font-bold text-indigo-900 mb-2">รวมแชทจากสาขาอื่น (Copy Settings)</h4><div className="bg-white/60 rounded-lg p-3 border border-indigo-100 mb-4"><p className="text-sm text-indigo-800 leading-relaxed">คัดลอกการตั้งค่า LINE (Channel Access Token และ Channel Secret)<br/>มาใช้กับสาขานี้ได้ทันที</p></div><div className="max-w-md"><Select value={defaultCommunicationBranch} onValueChange={(v) => {setDefaultCommunicationBranch(v);if (v === 'none') {setLineSettings({ line_channel_access_token: '', line_channel_secret: '' });return;}const branchConfigs = configs.filter(c => c.branch_id === v);const tokenConfig = branchConfigs.find(c => c.key === 'line_channel_access_token');const secretConfig = branchConfigs.find(c => c.key === 'line_channel_secret');if (tokenConfig) {setLineSettings({line_channel_access_token: tokenConfig.value,line_channel_secret: secretConfig?.value || ''});toast.info('คัดลอกการตั้งค่า LINE มาแล้ว กดบันทึกเพื่อยืนยัน');} else {toast.error('สาขาที่เลือกยังไม่ได้ตั้งค่า LINE');}}}><SelectTrigger className="bg-white border-indigo-300 shadow-sm h-12 text-base font-medium text-indigo-900"><SelectValue placeholder="-- คัดลอกตั้งค่า LINE จากสาขา --" /></SelectTrigger><SelectContent><SelectItem value="none">-- ไม่ระบุ (ตั้งค่าเอง) --</SelectItem>{branches.filter(b => b.id !== selectedBranch?.id).map(b => (<SelectItem key={b.id} value={b.id}>{b.branch_name}</SelectItem>))}</SelectContent></Select></div></div></div></div>
                        )}
