                    ...
                    <Tabs defaultValue="line" className="w-full">
                      ...
                      <TabsContent value="line" className="space-y-6">
                        ...
                        {(userRole === 'developer' || userRole === 'owner') && branches.length > 1 && (
                          <div className="bg-gradient-to-br from-indigo-50 to-blue-50 rounded-xl p-5 border border-indigo-100 shadow-sm relative overflow-hidden"><div className="absolute -right-4 -top-4 w-24 h-24 bg-indigo-100 rounded-full opacity-50 blur-xl"/><div className="flex items-start gap-4 relative z-10"><div className="bg-indigo-100 p-3 rounded-full flex-shrink-0 mt-1 shadow-sm border border-indigo-200"><MessageSquare className="w-6 h-6 text-indigo-600"/></div><div className="flex-1"><h4 className="text-lg font-bold text-indigo-900 mb-2">สาขาหลักสำหรับรับลูกค้าใหม่</h4><div className="bg-white/60 rounded-lg p-3 border border-indigo-100 mb-4"><p className="text-sm text-indigo-800 leading-relaxed">เมื่อมีผู้เช่าคนใหม่ทักเข้ามา จะถูกส่งไปที่ <b>สาขาที่คุณเลือก</b></p></div><div className="max-w-md"><Select value={defaultCommunicationBranch} onValueChange={async (v)=>{setDefaultCommunicationBranch(v);try{const c=await base44.entities.Config.filter({key:'default_communication_branch',branch_id:selectedBranch?.id},'',1);if(c&&c.length>0){if(v&&v!=='none')await base44.entities.Config.update(c[0].id,{value:v});else await base44.entities.Config.delete(c[0].id);}else if(v&&v!=='none')await base44.entities.Config.create({key:'default_communication_branch',value:v,branch_id:selectedBranch?.id,category:'notification'});await queryClient.refetchQueries({queryKey:['configs']});toast.success('บันทึกสำเร็จ');}catch(e){toast.error('บันทึกไม่สำเร็จ');}}}><SelectTrigger className="bg-white border-indigo-300 shadow-sm h-12 text-base font-medium text-indigo-900"><SelectValue placeholder="-- ใช้สาขาปัจจุบันเป็นค่าเริ่มต้น --"/></SelectTrigger><SelectContent><SelectItem value="none">-- ใช้สาขาปัจจุบันเป็นค่าเริ่มต้น --</SelectItem>{branches.map(b=><SelectItem key={b.id} value={b.id}>{b.branch_name}</SelectItem>)}</SelectContent></Select></div></div></div></div>
                        )}
