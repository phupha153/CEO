...
export default function Dashboard() {
  ...
  const recentPayments = useMemo(() => {
    ...
    return payments
      .filter(p => p.status === 'paid' && (p.receipt_sent_date || p.created_date))
      .sort((a, b) => {
        try {
          // ใช้ receipt_sent_date (เวลาส่งใบเสร็จ = เวลาชำระจริง) หรือ created_date
          // ไม่ใช้ updated_date เพราะเป็น auto-timestamp ที่ไม่ตรงกับเวลาชำระ
          const dateStrA = a.receipt_sent_date || a.created_date;
          const dateStrB = b.receipt_sent_date || b.created_date;
                                            ...
                                            <p className="text-xs text-slate-500">
                                              {(() => {
                                                try {
                                                  ...
                                                  const dateStr = receiptSentDateStr || createdDateStr;
