    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-100 via-blue-50 to-blue-100">
      ...
      <div className="px-4 md:px-8 py-3 md:py-6 relative z-10">
        <div className="max-w-7xl mx-auto space-y-3 md:space-y-6">
          ...
          <PaymentStatCards displayCounts={displayCounts} totalAmounts={totalAmounts} statusFilter={statusFilter} setStatusFilter={setStatusFilter} isLoading={viewMode === 'room' ? roomViewFetching : paymentsLoading} />{bookingTypeFilter !== 'monthly' && (<Card className="hidden md:block bg-white/60 backdrop-blur-2xl border border-white/80 shadow-2xl rounded-2xl md:rounded-3xl overflow-hidden"><div className="absolute top-0 right-0 w-48 md:w-64 h-48 md:h-64 bg-gradient-to-br from-blue-200/20 to-sky-200/15 rounded-full blur-3xl" /><CardContent className="p-4 md:p-6 relative"><PaymentsAISection searchQuery={searchQuery} setSearchQuery={setSearchQuery} handleAISearch={handleAISearch} handleStopAISearch={handleStopAISearch} aiSearching={aiSearching} aiAction={aiAction} handleAIActionConfirm={handleAIActionConfirm} handleAIActionCancel={handleAIActionCancel} aiActionLoading={aiActionLoading} aiResult={aiResult} payments={payments} getEffectiveStatus={getEffectiveStatus} calculateLateFee={calculateLateFee} handlePaymentClick={handlePaymentClick} /></CardContent></Card>)}
          ...
          />
          {bookingTypeFilter !== 'monthly' && (<Card className="bg-white/80 backdrop-blur-sm border-slate-200/60 shadow-lg sticky top-[73px] md:top-[85px] z-30"><CardContent className="p-3"><div className="flex gap-2 items-center"><div className="flex-1"><div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 w-4 h-4" /><Input placeholder="ค้นหาห้อง หรือผู้เช่า..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-9 bg-white/90 shadow-inner border-slate-200 h-10" />{searchQuery && (<button onClick={() => setSearchQuery('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600"><X className="w-4 h-4" /></button>)}</div></div><div className="w-[120px]"><Select value={statusFilter} onValueChange={setStatusFilter}><SelectTrigger className="w-full h-10 text-xs bg-white/90 shadow-inner border-slate-200"><SelectValue placeholder="สถานะ" /></SelectTrigger><SelectContent><SelectItem value="all">ทั้งหมด</SelectItem><SelectItem value="pending">รอชำระ</SelectItem><SelectItem value="overdue">เกินกำหนด</SelectItem><SelectItem value="paid">ชำระแล้ว</SelectItem></SelectContent></Select></div></div></CardContent></Card>)}
