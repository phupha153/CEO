...
export default function Announcements() {
  ...
  const { data: facebookMessages = [], isLoading: fbMessagesLoading, isFetching: fbFetching, error: fbMessagesError, refetch: refetchFacebookMessages } = useQuery({
    ...
    queryFn: async () => {
      ...
      // ⚡ Fetch แบบขนาน (Parallel) พร้อมดัก Error แยกแต่ละ Request ไม่ให้พังทั้งหมด
      const promises = [
          base44.entities.FacebookMessage?.filter({ branch_id: selectedBranchId }, '-created_date', 500).catch(e => { console.error('Error branch fb messages', e); return []; }),
          base44.entities.FacebookMessage?.filter({ branch_id: null }, '-created_date', 100).catch(e => { console.error('Error null fb messages', e); return []; }) // Always fetch unlinked
      ];

      // ถ้ามีสาขาหลัก และเราไม่ได้อยู่ที่สาขาหลัก -> ให้ดึงข้อความจากสาขาหลักมาด้วย (เฉพาะที่ยังไม่ผูกผู้เช่า)
      if (defaultBranchId && defaultBranchId !== 'none' && !isDefaultBranch) {
          promises.push(base44.entities.FacebookMessage?.filter({ branch_id: defaultBranchId }, '-created_date', 300).catch(e => { console.error('Error default branch fb messages', e); return []; }));
      }
      ...
      // กรองข้อความจากสาขาหลัก เอาเฉพาะที่ยังไม่มี tenant_id (ลูกค้าใหม่)
      let defaultBranchMsgs = [];
      if (results[2]) {
          const rawDefMsgs = Array.isArray(results[2]) ? results[2] : [results[2]];
          defaultBranchMsgs = rawDefMsgs.filter(m => !m.tenant_id);
