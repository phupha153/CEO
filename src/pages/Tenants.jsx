...
                    <p className="text-xs text-slate-500">
                      ลบเมื่อ: {record.deleted_date ? format(parseISO(record.deleted_date), 'd MMM yy HH:mm', { locale: th }) : '-'}
...
                  </div>
                  <Button size="sm" variant="outline" onClick={toggleSelectAllInPage} className="bg-blue-50 border-blue-300 text-blue-700">
                    <CheckSquare className="w-4 h-4 mr-1" /> ทั้งหมด ({displayedTenants.length})
                  </Button>
                  <Button size="sm" variant="outline" onClick={() => setSelectedTenants([])} className="bg-white border-slate-300">
                    <X className="w-4 h-4 mr-1" /> ล้าง
                  </Button>
                  <Button size="sm" variant="destructive" onClick={handleBulkDelete} disabled={isBulkExecuting} className="bg-red-600 font-bold">
                    {isBulkExecuting ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" /> กำลังลบ...</> : <><Trash2 className="w-4 h-4 mr-1" /> ลบ ({selectedTenants.length})</>}
                  </Button>
                </div>
...
                          <div className="mt-2 flex flex-wrap gap-2">
                            <Badge className={bulkAIResult.vehicle_type === 'car' ? 'bg-blue-100 text-blue-700' : 'bg-green-100 text-green-700'}>
                              {bulkAIResult.vehicle_type === 'car' ? '🚗 รถยนต์' : '🏍️ มอเตอร์ไซค์'}
                            </Badge>
                            {bulkAIResult.vehicle_plate && (
                              <Badge variant="outline" className="bg-white">
                                ทะเบียน: {bulkAIResult.vehicle_plate}
                              </Badge>
                            )}
                            {bulkAIResult.vehicle_brand && (
                              <Badge variant="outline" className="bg-white">
                                ยี่ห้อ: {bulkAIResult.vehicle_brand}
                              </Badge>
                            )}
                            {!bulkAIResult.vehicle_plate && !bulkAIResult.vehicle_brand && (
...
                        disabled={isBulkExecuting}
                        className={bulkAIResult.action === 'delete' ? 'bg-red-600 hover:bg-red-700' : bulkAIResult.action === 'add_vehicle' ? 'bg-purple-600 hover:bg-purple-700' : 'bg-green-600 hover:bg-green-700'}
                      >
                        {isBulkExecuting ? (
                          <>
                            <Loader2 className="w-4 h-4 mr-2 animate-spin" />
