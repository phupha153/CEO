...
async function handleSlipImage(base44, lineUserId, messageId, branchId = null, replyToken = null) {
    ...
    try {
        ...
        // ⭐ FIX: sendReceipt ก่อน (ใช้ replyToken), ข้อความ partial/excess ใช้ push (null)
        let rT = replyToken;
        for (const item of processedIds) { if (item.status === 'paid') { try { await base44.asServiceRole.functions.invoke('sendReceipt', { paymentId: item.id, replyToken: rT }); rT = null; } catch (e) { console.error(`❌ sendReceipt failed for ${item.id}:`, e.message); } } }
