...
Deno.serve(async (req) => {
    ...
    try {
        ...
        const format = url.searchParams.get('format') || 'json'; // 'json' or 'html'

        console.log(`📋 Payment ID: ${paymentId}, Type: ${type}, Format: ${format}`);
        ...
        // ⭐ ถ้าต้องการ JSON (สำหรับ React pages) - return แบบเดิม
        if (format === 'json') {
            const invoiceObject = {
                id: payment.id,
