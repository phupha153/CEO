...
async function handleSlipImage(base44, lineUserId, messageId, branchId = null, replyToken = null) {
    ...
    try {
        ...
        if (!nameMatch) {
            console.log('❌ NAME MISMATCH - Manual review required');
            ...
            const errorMsg = `ชื่อบัญชีไม่ตรง (ตรวจพบ: ${receiverName}, ควรเป็น ${expectedAccountName})`;
            const lineMessage = `❌ ชื่อบัญชีไม่ตรง!\n\nคุณโอนเข้า: ${receiverName}\nควรเป็น: ${expectedAccountName}\n\nกรุณาตรวจสอบหรือติดต่อเจ้าของหอพักค่ะ 🙏`;
        ...
        await sendMessage(base44, lineUserId, 'เกิดข้อผิดพลาดในการประมวลผล กรุณาลองใหม่อีกครั้ง', errorTenant?.branch_id || branchId, replyToken);
    }
}
...
async function handleEmployeeExpenseSubmission(base44, lineUserId, employee, messageText, replyToken, branchId) {
    try {
        ...
        // ⭐⭐⭐ ถ้ามี pending data ที่มีรูปอยู่แล้ว (ส่งรูปก่อน แล้วส่งข้อความตาม)
        if (pendingData && pendingData.receipt_image) {
            const pendingCreatedAt = pendingData.created_at ? new Date(pendingData.created_at) : new Date(0);
            const nowTime = new Date();
            const secondsSincePending = (nowTime - pendingCreatedAt) / 1000;
            
            console.log(`⏱️ Pending data with image exists - Age: ${secondsSincePending.toFixed(1)}s`);
            
            // ⭐ ถ้าส่งภายใน 30 วินาที → Auto-merge (ใช้ข้อความ + รูปเก่า)
            if (secondsSincePending <= 30) {
                console.log(`✅ Within 30s window - AUTO-MERGING image (from pending) + text (new)`);
                
                // AI Extract จากข้อความ (เฉพาะ category, title, description)
                const textAnalysis = await base44.asServiceRole.integrations.Core.InvokeLLM({
                    prompt: `วิเคราะห์ข้อความค่าใช้จ่ายนี้:
...
3. description: รายละเอียดสั้นๆ`,
                ...
                // ผสานข้อมูล: category/title/description จากข้อความ + amount/date/image จากรูปเดิม
                const mergedData = {
                ...
                console.log('✅ Auto-merged text + image:', mergedData);
                
                // ส่ง Flex confirmation
                await sendFlexConfirmation(base44, lineUserId, mergedData, categoryTh, branchId, replyToken);
                return;
            } else {
                console.log(`⚠️ Outside 30s window - treating as new expense (will overwrite old data)`);
                // เกิน 30 วินาที → ลบข้อมูลเก่า แล้ว analyze ข้อความใหม่ต่อด้านล่าง
                await base44.asServiceRole.entities.User.update(employee.id, {
                    expense_pending_data: null
                });
            }
