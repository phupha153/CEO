        ...
        try {
            ...
            for (const event of events) {
                ...
                if (event.type === 'postback') {
                    ...
                    // ⭐ Log postback events (maintenance, registration, etc.)
                    try {
                        await base44.asServiceRole.entities.WebhookLog.create({
                            ...
                            status: 'warning',
                            message: `Postback event: ${postbackData}`,
                            details: { postback_data: postbackData }
                        }).catch(() => {});
                    } catch (logError) {
                        // Silent fail
                    }
                ...
                if (event.type === 'message') {
                    ...
                    const hasMaintenanceKeyword = maintenanceKeywords.some(kw => messageText.includes(kw));
                    const hasExpenseKeyword = expenseKeywords.some(kw => messageText.includes(kw));
...
async function handleSlipImage(base44, lineUserId, messageId, branchId = null, replyToken = null) {
    ...
    try {
        ...
        if (slipAmount === 0) {
             try {
                await base44.asServiceRole.entities.WebhookLog.create({
                    ...
                    payment_id: pendingPayment.id,
                    status: 'warning',
                    message: 'Could not read amount from slip',
                    details: { slipData }
                });
            } catch(logError) { /* silent fail */ }
        ...
        if (totalPaid < expectedAmount * 0.95) {
            ...
            try {
                await base44.asServiceRole.entities.WebhookLog.create({
                    ...
                    amount: slipAmount,
                    status: 'warning',
                    message: `Amount mismatch: paid ${totalPaid}, expected ${expectedAmount}`,
                    details: { shortfall, late_fee: lateFeeAmount, days_late: daysLate }
                });
            } catch(logError) { /* silent fail */ }
            const shortfall = expectedAmount - totalPaid;
        ...
        try {
            ...
            // Silent fail - even if logging fails
        }
